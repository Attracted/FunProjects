...
namespace TwinCars
{
   class Particle
   {
      ...
      public Turning Spin { get; set; }

      public Particle(Bitmap image, float x, float y, float xDrift, float angle, Turning spin)
      {
         Image = image;
         XAxis = x;
         YAxis = y;
      ...
      public Bitmap GetImage(out float x, out float y)
      {
         x = XAxis + XDrift;
         y = YAxis;
         if (Spin == Turning.Left)
         {
            Angle -= 5;
         }
         else if (Spin == Turning.Right)
         {
            Angle += 5;
         }
